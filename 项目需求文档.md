...
CREATE TABLE learning_resource (
    ...
    visibility ENUM('ALL', 'COURSE', 'CLASS') DEFAULT 'ALL' COMMENT '可见性',
...
1. **用户(SYS_USER) → 学习资源(LEARNING_RESOURCE)**
   - 关系：一个用户可以上传多个学习资源
   - 外键：`learning_resource.uploader_id` → `sys_user.id`
   - 约束：级联删除

2. **用户(SYS_USER) → 问题(QUESTION)**
   - 关系：一个学生可以提出多个问题
   - 外键：`question.student_id` → `sys_user.id`
   - 约束：级联删除

3. **用户(SYS_USER) → 回答(ANSWER)**
   - 关系：一个教师可以回答多个问题
   - 外键：`answer.teacher_id` → `sys_user.id`
   - 约束：级联删除

4. **用户(SYS_USER) → 通知(NOTIFICATION)**
   - 关系：一个用户可以接收多条通知
   - 外键：`notification.user_id` → `sys_user.id`
   - 约束：级联删除

5. **用户(SYS_USER) → 收藏(COLLECTION)**
   - 关系：一个用户可以收藏多个资源
   - 外键：`collection.user_id` → `sys_user.id`
   - 约束：级联删除

6. **课程(COURSE) → 学习资源(LEARNING_RESOURCE)**
   - 关系：一个课程下可以有多个学习资源
   - 外键：`learning_resource.course_id` → `course.id`
   - 约束：级联删除

7. **课程(COURSE) → 问题(QUESTION)**
   - 关系：一个课程下可以有多个问题
   - 外键：`question.course_id` → `course.id`
   - 约束：级联删除

8. **问题(QUESTION) → 回答(ANSWER)**
   - 关系：一个问题可以有多个回答
   - 外键：`answer.question_id` → `question.id`
   - 约束：级联删除

9. **学习资源(LEARNING_RESOURCE) → 收藏(COLLECTION)**
   - 关系：一个资源可以被多个用户收藏
   - 外键：`collection.resource_id` → `learning_resource.id`
   - 约束：级联删除
...
1. **教师(SYS_USER) ↔ 课程(COURSE)**
   ...
   - 中间表：`teacher_course`
   - 外键1：`teacher_course.teacher_id` → `sys_user.id`
   - 外键2：`teacher_course.course_id` → `course.id`
...
- `POST /api/v1/resources/{id}/collect` - 收藏资源
- `DELETE /api/v1/resources/{id}/collect` - 取消收藏
- `GET /api/v1/resources/my-uploads` - 我上传的资源
- `GET /api/v1/resources/my-collections` - 我的收藏
